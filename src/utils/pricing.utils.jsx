...
export function buildPlanFeatures(plan) {
    ...
    if (typeof plan.export_limit === 'number') {
...
export function getPlanRoute(planSlug, isAuthenticated) {
    const unauthRoutes = {
        free: config.router.register,
        premium: config.router.login,
    };
    // thêm router payment khi payment làm xong để chuyển qua checkout
    const authRoutes = {
        free: config.router.cvTemplates,
        premium: config.router.payment,
    };

    const routes = isAuthenticated ? authRoutes : unauthRoutes;

    return routes[planSlug] ?? config.router.home;
}