...
const normalizeInsightsMeta = (meta) => {
  ...
  const hiddenCount = normalizeMetaCount(meta.hidden_count ?? meta.hiddenCount);
  ...
  data: {
    ...
    weaknesses: [
      {
        ...
        evidence: {
          ...
          reason:
            "Thiếu kinh nghiệm thực tế có thể ảnh hưởng đến khả năng thích ứng với môi trường làm việc chuyên nghiệp.",
        },
    ...
    suggestions: [
      ...
      },
      {
        ...
        action:
          "Mô tả rõ hơn cách đã thiết kế và phát triển API trong các dự án cá nhân.",
      },
    ],
    suggestions_meta: {
      visible_count: 2,
    ...
    weaknesses: [
      {
        ...
        evidence: {
          ...
          reason:
            "Ứng viên chưa có kinh nghiệm làm việc thực tế, điều này có thể ảnh hưởng đến khả năng thích ứng với quy trình làm việc chuyên nghiệp và áp lực dự án thực tế.",
        },
        target_section: "experience",
      },
      {
        ...
        evidence: {
          ...
          reason:
            "Ứng viên chỉ thể hiện kinh nghiệm với MongoDB, có thể thiếu kiến thức về các hệ quản trị CSDL quan hệ phổ biến khác, mặc dù JD cho phép lựa chọn.",
        },
      ...
      },
      {
        ...
        evidence: {
          ...
          reason:
            "Mặc dù sử dụng Express.js cho backend, CV không mô tả chi tiết về việc thiết kế, phát triển hay duy trì API theo chuẩn RESTful hoặc GraphQL, chỉ tập trung vào các tính năng.",
        },
        target_section: "experience",
      },
      {
        ...
        evidence: {
          ...
          reason:
            "Ứng viên chưa thể hiện kinh nghiệm hoặc kiến thức về việc tối ưu hóa hiệu năng hệ thống thông qua CSDL và truy vấn.",
        },
        target_section: "experience",
      },
      {
        ...
        evidence: {
          ...
          reason:
            "Ứng viên chưa có kinh nghiệm hoặc chưa thể hiện khả năng viết tài liệu kỹ thuật, một yêu cầu trong JD.",
        },
        target_section: "experience",
      },
      {
        ...
        evidence: {
          ...
          reason:
            "Ứng viên chưa thể hiện kinh nghiệm làm việc với GraphQL, một công nghệ API được đề cập trong JD.",
        },
        target_section: "skills",
      },
    ],
    suggestions: [
      {
        title: "Bổ sung kinh nghiệm thực tế",
        action:
          "Tìm kiếm các cơ hội thực tập tại các công ty có dự án thực tế để tích lũy kinh nghiệm làm việc trong môi trường chuyên nghiệp.",
      },
      {
        title: "Mở rộng kiến thức CSDL",
        action:
          "Tìm hiểu và thực hành với các hệ quản trị cơ sở dữ liệu quan hệ như MySQL hoặc PostgreSQL song song với MongoDB.",
      },
      {
        title: "Chi tiết hóa kinh nghiệm API",
        action:
          "Trong CV, mô tả rõ hơn về cách đã thiết kế, phát triển và duy trì các API RESTful trong các dự án cá nhân, nêu bật các chuẩn và nguyên tắc đã áp dụng.",
      },
      {
        title: "Thêm kinh nghiệm tối ưu hóa hiệu năng",
        action:
          "Trong phần mô tả dự án, có thể bổ sung các kỹ thuật đã áp dụng để tối ưu hóa truy vấn hoặc hiệu năng cơ sở dữ liệu (nếu có).",
      },
      {
        title: "Phát triển kỹ năng viết tài liệu",
        action:
          "Bắt đầu thực hành viết tài liệu kỹ thuật cho các dự án cá nhân, mô tả kiến trúc, API, hoặc các tính năng quan trọng.",
      },
      {
        title: "Tìm hiểu về GraphQL",
        action:
          "Nếu có hứng thú, nên tìm hiểu và thực hành với GraphQL để mở rộng phạm vi kỹ năng theo yêu cầu của JD.",
      },
    ],
    strengths: [
      {
        ...
        detail:
          "Ứng viên thể hiện kinh nghiệm làm việc với Node.js và Express.js thông qua hai dự án cá nhân SrcPlace và LearnEnglish, bao gồm các tính năng như xác thực JWT, tích hợp thanh toán, CRUD.",
      },
      {
        point: "Kinh nghiệm làm việc với MongoDB",
        detail:
          "CV nêu rõ việc sử dụng MongoDB làm cơ sở dữ liệu cho cả hai dự án, cho thấy sự quen thuộc với hệ thống NoSQL.",
      },
      {
        point: "Kỹ năng Frontend cơ bản",
        detail:
          "Ứng viên có đề cập đến các kỹ năng Frontend như HTML, CSS(SCSS), JavaScript và React.js, cho thấy khả năng làm việc full-stack ở mức độ nhất định.",
      },
      {
        point: "Sử dụng Git hiệu quả",
        detail:
          "CV mô tả việc sử dụng Git với các nhánh như 'main', 'develop', 'feature/....' và 'gitFlow', cho thấy hiểu biết về quản lý mã nguồn.",
      },
      {
        point: "Mục tiêu nghề nghiệp rõ ràng",
        detail:
          "Ứng viên có mục tiêu ngắn hạn và dài hạn rõ ràng, thể hiện mong muốn học hỏi và phát triển bản thân trong lĩnh vực Backend.",
      },
    ],
    structured_feedback: {
      summary_feedback:
...
const normalizeSuggestion = (item, index) => {
  ...
  return {
    ...
    example:
      item?.example ?? item?.action ?? item?.detail ?? item?.description ?? "",
  };
};
...
export const normalizeAiResult = (raw) => {
  ...
  return {
    ...
    suggestions: normalizeArray(source.suggestions).map(normalizeSuggestion),
    structuredFeedback:
...
export const normalizeTier = (value) => {
  const normalized = String(value || "")
    .trim()
    .toLowerCase();
  if (["premium", "pro", "paid", "plus", "vip", "gold"].includes(normalized)) {
    return "premium";
  }
  return "free";
};
...
export const getPriorityMeta = (priority) => {
  ...
  if (normalized === "high") return { label: "Ưu tiên cao", className: "high" };
  if (normalized === "low") return { label: "Ưu tiên thấp", className: "low" };
