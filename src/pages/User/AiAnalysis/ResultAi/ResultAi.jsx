...
function ResultAiPremium() {
  ...
  const statePayload = useMemo(() => {
    ...
    return (
      state.aiResult || state.result || state.data || state.payload || null
    );
  }, [location?.state]);
  ...
  //   );
  //   const [loading, setLoading] = useState(!statePayload && Boolean(aiRunId));

  // 1. Ép luôn lấy dữ liệu mẫu từ MOCK_DB_RESULT
  const [aiResult, setAiResult] = useState(
    normalizeAiResult(MOCK_DB_RESULT_Premium.data),
  );

  // 2. Ép tắt loading để UI hiện ra ngay lập tức
  const [loading, setLoading] = useState(false);

  const [errorMessage, setErrorMessage] = useState("");

  const loadResult = useCallback(async (runId, { silent, signal } = {}) => {
    if (!runId) return;

    setErrorMessage("");
    setLoading(true);
    try {
      const result = await getAiAnalysisResultByRunId(runId, { signal });
      if (!result?.success) {
        if (signal?.aborted) return;
        const message = result?.message || "Không thể tải kết quả phân tích";
      ...
      if (signal?.aborted) return;
      setAiResult(normalizeAiResult(result.data));
    } catch (error) {
      if (signal?.aborted) return;
      const message = error?.message || "Không thể tải kết quả phân tích";
      setErrorMessage(message);
      if (!silent) toast.error(message);
    } finally {
  ...
  useEffect(() => {
    // if (statePayload) return undefined;
    // if (!aiRunId) return undefined;
    // const abortController = new AbortController();
    // loadResult(aiRunId, { silent: true, signal: abortController.signal });
    // return () => abortController.abort();
  }, [aiRunId, statePayload, loadResult]);

  const resolvedTier = useMemo(() => {
    const state = location?.state ?? {};
    const tierFromState =
      state?.tier ?? state?.planTier ?? state?.packageTier ?? state?.userTier;
  ...
  const editCvHref = useMemo(() => {
    if (cvId != null && String(cvId).trim() !== "") {
      return `${config.router.cvEdit}/${cvId}`;
    }
    return config.router.cvEdit;
  }, [cvId]);
      ...
      <div className={cx("pageShell")}>
            ...
            <div className={cx("titleBlock")}>
              ...
              <div>
                ...
                <p className={cx("subtitle")}>
                  Tập trung vào đúng vấn đề khiến CV rớt ATS/recruiter và ưu
                  tiên hành động theo tác động.
                </p>
              </div>
            </div>

            <div className={cx("metaBlock")}>
              <span className={cx("tierPill", isPremium ? "premium" : "free")}>
        ...
        {errorMessage ? (
          <div className={cx("errorCard")}>
            ...
            <div className={cx("errorBody")}>
              <div className={cx("errorTitle")}>Không thể tải kết quả</div>
              <div className={cx("errorText")}>{errorMessage}</div>
            </div>
            <button
              type="button"
        ...
        {loading ? (
            ...
            <section className={cx("card")}>
              ...
              {!isPremium && hiddenStrengthCount > 0 ? (
                <div className={cx("lockInline")}>
                  <FiLock />
                  <span>
                    Còn {hiddenStrengthCount} điểm mạnh chưa mở khóa trong gói
                    FREE.
                  </span>
                </div>
              ) : null}
                    ...
                    return (
                      <li key={item.id} className={cx("weaknessItem")}>
                        ...
                        <div className={cx("itemMeta")}>
                          {item.targetSection ? (
                            <span>Section: {item.targetSection}</span>
                          ) : null}
            ...
            <section className={cx("card", "wide")}>
                      ...
                      <li key={item.id} className={cx("suggestionItem")}>
                        ...
                        {item.action || item.example ? (
                          <div className={cx("suggestionExample")}>
                            Hành động:{" "}
                            {stringifyValue(item.action || item.example)}
                          </div>
                        ) : null}
                      </li>
                    );
              ...
              {!isPremium && hiddenSuggestionCount > 0 ? (
                <div className={cx("lockInline")}>
                  <FiZap />
                  <span>
                    Còn {hiddenSuggestionCount} gợi ý nâng cấp CV chuyên sâu cho
                    Premium.
                  </span>
                </div>
              ) : null}
            ...
            <section className={cx("card", "wide")}>
                  ...
                  {visibleStructuredFeedback.map(([key, value]) => (
                    <details key={key} className={cx("accordionItem")}>
                      <summary className={cx("accordionSummary")}>
                        ...
                        <FiChevronDown className={cx("accordionIcon")} />
                      </summary>
              ...
              {!isPremium && hiddenStructuredCount > 0 ? (
                <div className={cx("lockInline")}>
                  <FiTrendingUp />
                  <span>
                    Còn {hiddenStructuredCount} phân tích recruiter-view chỉ có
                    ở Premium.
                  </span>
                </div>
              ) : null}
          ...
          <div className={cx("emptyCard")}>
            ...
            <div>
              <h3 className={cx("emptyTitle")}>Chưa có dữ liệu kết quả</h3>
              <p className={cx("emptyText")}>
                Hãy thực hiện phân tích CV để xem báo cáo tại đây.
              </p>
            </div>
          </div>
        )}
