...
export function ProfileSection({ section, data, content, layoutType }) {
            ...
            <div className={cx('profileBanner')}>
                <div className={cx('profileBannerContent')}>
                    <div className={cx('profileBannerInfo')}>
                        {name ? (
                            <div className={cx('fullName', 'fullNameLight')}>
                                {name}
                            </div>
                        ) : null}
                        {headline ? (
                            <div className={cx('headline', 'headlineLight')}>
                                {headline}
                            </div>
                        ) : null}
                    </div>
    ...
    return (
        <div className={cx('profileSidebar')}>
            ...
            {headline ? (
                <div className={cx('headlinePill')}>{headline}</div>
            ) : null}
            ...
            <div className={cx('skillsList')}>
                {data.map((item, index) => {
                    const level = Math.max(
                        0,
                        Math.min(Number(item?.level || 0), 100),
                    );
        ...
        return (
            <div className={cx('skillsList')}>
                {data.map((item, index) => {
                    const level = Math.max(
                        0,
                        Math.min(Number(item?.level || 0), 5),
                    );
                    ...
                    <div key={index} className={cx('cardItem')}>
                        <div className={cx('eduHeader')}>
                            <div className={cx('eduMain')}>
                                {item?.degree ? (
                                    <div className={cx('eduDegree')}>
                                        {item.degree}
                                    </div>
                                ) : null}

                                {item?.school ? (
                                    <div className={cx('eduSchool')}>
                                        {item.school}
                                    </div>
                                ) : null}
                            </div>
        ...
        <div className={cx('cardList')}>
            {data.map((item, index) => {
                ...
                return (
                    <div
                        key={index}
                        className={cx('cardItem', { cardFlat: leftDate })}
                    >
                        {leftDate ? (
                    ...
                    <div key={index} className={cx('cardItem')}>
                        <div className={cx('expHeader')}>
                            <div className={cx('expMain')}>
                                {item?.role ? (
                                    <div className={cx('expRole')}>
                                        {item.role}
                                    </div>
                                ) : null}

                                {item?.company ? (
                                    <div className={cx('expCompany')}>
                                        {item.company}
                                    </div>
                                ) : null}
                            </div>
